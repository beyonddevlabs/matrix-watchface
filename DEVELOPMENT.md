...
The technical side of Matrix Watchface. The user-facing and installation side
...
```bash
msedge --headless=new --window-size=51,76   --screenshot=.../image/flap/big/roll_9_3.png   "file:///<path>/design/frames/flap.html?size=big&from=9&f=3&c=c9ffd9&g=0,255,65"
```

`size` picks the cell size, `from` the starting digit, `f` the single frame, `c`
the digit colour and `g` the glow colour as an RGB triple. The red variant uses
the same page with different colour values.
...
A black background and a single green colour are the cheapest case on AMOLED —
...
- **Colour format.** `@zos/ui` takes colours as a plain number (`0x00ff41`), the
